...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="product-card">
    <div className="product-image"><span>Menu</span><Image src={product.image} alt={`${product.name} from the Smoked 1804 menu`} width={430} height={350} /></div>
    <div className="stars" aria-hidden="true">{[1,2,3,4,5].map(n => <Star key={n} fill="currentColor" />)}</div>
